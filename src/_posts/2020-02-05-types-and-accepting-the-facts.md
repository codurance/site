...
So why does the last two ones are done over the integer value? In JavaScript, strings have the `+`
...
We still enforce neither parameter nor return type, but what Ruby does is try to be more conservative with castings, when an operation doesn't sound right Ruby will thrown an error instead of trying to make the operation to happen.  
...
Well, not exactly. Compile-time check gives you a guard rail against some problems. Said that relying on types purely to avoid problems isn't the best way to go, even with statically typed languages you are bound to commit mistakes as the one shown previously. 
...
Everyone is a close friend of nulls, from bankrupting companies and making devs to drink under their desks, they are everywhere. Inevitable like making bad decisions when you are drunk, we need to deal with nulls. 
...
Everything that I wrote in this post is more an exercise than a rule set to stone, especially because I'm not in a position to define what people should do, I wish this post can be a starting point to know more about types.
