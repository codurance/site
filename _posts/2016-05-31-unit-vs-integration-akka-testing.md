...
* Unit: the key in here is isolation. Some people talks about isolated production code, e.g. a single function or a bunch of methods within a class (one public and the rest private). [Some other people](https://vimeo.com/68375232) talks about isolated tests, i.e. tests that may be executed on parallel as they don't access to any shared resource. A third view is that a unit test execution should be synchronous without any concurrency concern. That's [Akka's view](http://doc.akka.io/docs/akka/current/scala/testing.html):
...
* Inspect the internal state. This might be an smell on some cases.
...
```scala
"Item FSM" should {
  "move into active state when receiving an item" in {
...
I want to share something that confused me a little the first time, but it's important to understand. We need to recall that unit testing in Akka means using one single thread in order to achieve deterministic order of events. `TestActorRef` uses `CallingThreadDispatcher` by default. This dispatcher *runs invocations on the current thread only* so we could 'check the returned value' of an actor with this unit test style.
...
Akka provides `TestKit` class for integration testing. Let's see one of our tests written using that class:

```scala
class ItemFSMSpec() extends TestKit(ActorSystem("ItemFSMSpec")) with ImplicitSender

"send complete message to the original sender when one item deleted is received and there are not more pending" in {
...
Here we're saying: send a message of `ItemReported` type to the actor assigned in the val `worker`. But who is sending that message? Extending and mixing `TestKit` and `ImplicitSender` creates a `testActor` that will be the messages sender. `TestKit` exposes some methods like `expectMsg` to inspect the mailbox of that `testActor`. `within` acts like [Scalatest `eventually`](http://www.artima.com/docs-scalatest-2.0.M5/org/scalatest/concurrent/Eventually.html) but with more power. For instance, as the documentation says:
...
Another interesting class is `TestProbe`. If we have several actors in our integration test and we want to verify different messages, using a single `testActor` might be confusing. Even with a single actor, using a `TestProbe` improves readability in some cases:
...
In [the previous post](http://codurance.com/2016/05/16/coordination-in-akka/) we introduce that actor factory in order to create an actor pool. In here the `TestProbe` makes more clear who is sending and expecting the messages.

## Summary

Akka has testability as a main asset. The biggest challenge is understanding what we want to test: internal business logic of the actor or the async exchange of messages between different actors.
